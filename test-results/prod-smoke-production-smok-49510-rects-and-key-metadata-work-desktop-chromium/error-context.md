...
- Name: prod-smoke.spec.ts >> production smoke >> homepage, navigation, redirects, and key metadata work
- Location: tests/e2e/prod-smoke.spec.ts:7:7
...
Locator:  locator('section[aria-label="Marketplace verticals"] a').or(locator('.category-bar-grid a'))
Expected: 4
Received: 0
Timeout:  5000ms
...
Call log:
  ...
  - waiting for locator('section[aria-label="Marketplace verticals"] a').or(locator('.category-bar-grid a'))
